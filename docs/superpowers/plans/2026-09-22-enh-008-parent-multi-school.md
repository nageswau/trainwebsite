...
- Backend tests require the project's Postgres stack already running via the user's own `docker compose` — do not start or stop it yourself; if a test run fails with a connection error, ask the user to confirm the stack is up rather than starting it.
- Run backend tests from `apps/api/`: `python -m pytest tests/<file>.py -v` (optionally `-k <test_name>` for one test). `testpaths = ["tests"]` and `asyncio_mode = "strict"` are already configured in `apps/api/pyproject.toml`.
...
Run: `python -m pytest tests/test_enh_005_scope.py::test_adding_a_student_at_school_a_can_use_a_parent_already_linked_at_school_b -v` (from `apps/api/`)
...
Run: `python -m pytest tests/test_enh_005_scope.py::test_adding_a_student_at_school_a_can_use_a_parent_already_linked_at_school_b -v`
Expected: PASS

- [ ] **Step 5: Verify the AC4 regression pin still passes unchanged**

Run: `python -m pytest tests/test_sch_roster_parent_invite.py::test_parent_email_belonging_to_a_non_parent_account_is_rejected -v`
...
Run: `python -m pytest tests/test_enh_005_scope.py::test_link_parent_accepts_a_parent_already_linked_at_another_school -v`
...
Run: `python -m pytest tests/test_enh_005_scope.py -v` (whole file) and `python -m pytest tests/test_sch_roster_parent_invite.py -v` (whole file)
...
Run: `python -m pytest tests/test_enh_005_scope.py -k "unlinked or two_schools or zero_links" -v`
...
Run: `python -m pytest tests/test_enh_005_scope.py -v` (whole file)
...
Run: `python -m pytest tests/test_sch_roster_parent_invite.py::test_accepting_a_parent_invite_does_not_set_profile_school_id -v`
...
Run: `python -m pytest tests/test_sch_roster_parent_invite.py -v` (whole file)
...
Run: `python -m pytest tests/test_enh_005_approve.py tests/test_enh_005_concurrency.py -v`
