...
| 11 | Task 3b (`PATCH /auth/me`) was a normal task. | It changes authentication logic and needed the user's approval. | Approved and implemented 2026-09-20 (5 tests; a mutation check on its log line). `profile.university_id` has the same weakness and was deliberately left alone *(superseded: fixed after the code review, row 20)*. |
...
| 20 | The independent code review's four findings are all valid as stated. | Each was checked against the code and the repo's rules (`AGENTS.md`) before acting. **(High) `profile.university_id`: VALID.** A University Rep's access is decided by it (`workflows.py:158,1824`, `portal.py:929`, `inbound.py:139`), only an admin route should set it, and `PATCH /auth/me` let the rep change their own. It is outside ENH-004's own requirement (it is the Overseas portal), but it is the same endpoint and guard this feature already hardened, so the fix is one entry in `SERVER_OWNED_PROFILE_KEYS`. **(Medium) e2e mutates the shared active year: VALID.** `AGENTS.md`: never mutate a shared record other specs assume. **(Medium) traceability: VALID.** `AGENTS.md` requires RTM and catalogue updates; `ENH-003` has an RTM row and a `SECURITY_CONTROLS.md` entry that ENH-004 lacked, and `screen_catalog.json` lacked the two new screens. **(Low) QA evidence not inspectable: PARTLY VALID.** The Playwright spec is in the repo and re-runnable, but the exploratory pass is not, and the backlog wording was stale. | (High) 3 tests, 2 red before the change (the third, echo-unchanged, must and did already pass); `SERVER_OWNED_PROFILE_KEYS = ("school_id", "university_id")`; contract, decision register, spec and this plan updated where they said `university_id` was left alone. (Medium) The spec records the year it creates and `afterEach` closes it and asserts the set of active years is what it was before (a closed year cannot be reopened, and activating one does not close the others, so closing the run's own year is an exact restore). Verified by a run with a real baseline year active. (Medium) `RTM.md` row, `screen_catalog.json` `SCR-SCH-027`/`028` (text insertion, +47 lines, no other byte changed; the older `SCR-SCH-022`..`026` gap is pre-existing and left alone), `SECURITY_CONTROLS.md` rows, and `SCR-SCH-027` corrected for the QA fixes. (Low) `docs/quality/ENH-004_BROWSER_QA_2026-09-20.md` records method, environment, findings and limits; the backlog status no longer says "pending independent review" or that all related specs pass. Raw screenshots/scripts are deliberately not committed (binary bulk, local paths). |
| 21 | Verified as complete after the code review and the refactor. | The pre-completion verification (fresh runs of every gate) found three things earlier runs had missed. **(a)** Four of ENH-004's own log tests failed in the *full* suite (they passed alone): ENH-001's in-process migration disables `app.*` loggers. **(b)** ENH-004's new test file was unformatted under CI's `ruff format --check` (which was already red at base). **(c)** Two responsive defects that overflow metrics could not see, found by looking at screenshots: the global `.search { min-width: 240px }` pushed the label input out of its card at 320px and over the status text at 844x390, and controls were 15px on touch devices. | (a) Autouse `_app_loggers_enabled` fixture (the convention `test_enh_003_*` uses); reproduced red by running ENH-001 first, green after. (b) Formatted only that file; the repo-wide count is back to the base's 55. (c) `min-width: 0` on the row controls and `font-size: 16px` under `pointer: coarse`, in `SchoolPromotionPanel.module.css`; red then green over 14 viewports. Also corrected my own measurement error (scripted `.focus()` is not keyboard focus). Final evidence and its limits: `docs/quality/ENH-004_BROWSER_QA_2026-09-20.md`. |
...
**Deliberately not changed *(at the time; `university_id` was later added, execution-log row 20)*:** other profile keys, notably `university_id` (the same bypass class for university reps, `workflows.py:158,1824`, `inbound.py:138`, `portal.py:929`) — that belongs to UNI-001 and is reported to the user as a separate finding, not silently widened here. The admin route `PATCH /admin/users/{id}` (which legitimately sets `profile`) is untouched.
...
`RTM.md` is intentionally not touched (its own header says ENH-001/ENH-002 are not tracked there; ENH-003 was, at that spec's explicit direction). `docs/ux/screen_catalog.json` is not touched (it does not contain `SCR-SCH-025` either, so it is already out of step with the Markdown catalogue). *(Both decisions were reversed after the independent code review: `AGENTS.md` requires the RTM and both catalogues to be kept current, so `RTM.md`, `screen_catalog.json` (`SCR-SCH-027`/`028` only; the older `SCR-SCH-022`..`026` gap is pre-existing and left alone) and `SECURITY_CONTROLS.md` were updated. See execution-log row 20.)*
...
**`NEEDS_CONFIRMATION` (not decided here):** graduate/alumni handling after Grade 12; a real section field; whether admin roles should ever promote; parent notification on promotion; promotion events in the SCH-008 timeline. Security follow-ups found during review and outside this feature: `profile.university_id` could be changed by its owner through `PATCH /auth/me` (the same class as the `school_id` exposure; UNI-001; fixed after the code review, row 20); no school-student erasure path exists; no app-wide rate limiter or CSRF token.
```
