...
- **Admin page (browser QA N3, revised 2026-09-21):** the admin screen was first built as the portal's generic section (`PortalPage` needs a `PORTAL_NAV` entry and a backend payload, so `GET /portal/overseas/admin/school-transfers` returned a table of the 200 most recent requests). That table (raw IDs and ISO timestamps, a different count from the queue) sat above the queue and pushed the approve/reject controls below the fold. It is now a dedicated page, `app/overseas/admin/school-transfers/page.tsx` (a static route beats `[section]`; the nav entry is unchanged), with an admin-role guard, a title and the queue; the portal payload and its `WorkflowPanel` wiring were removed.
- **Request form position (browser QA N4, revised 2026-09-21):** §7.1 put the "Request a transfer" disclosure last on the student page; on a phone that was a long scroll past the timeline. It is now the card directly under the student header, still collapsed.
