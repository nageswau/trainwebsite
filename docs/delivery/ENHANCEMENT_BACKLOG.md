...
**Status (2026-09-20).** Designed and decided in `docs/superpowers/specs/2026-09-19-enh-004-student-promotion-design.md` (`DEC-SCOPE-020`); implemented on branch `feature/enh-004-student-grade-promotion`, **pending browser validation and independent review**. The line references above (`models.py:221,992,1157`, `schools.py:1145`) were stale when this item was drafted; the current definitions are `SchoolStudent` in `models.py` and `bulk_upload_students` in `schools.py`. The endpoint paths differ from the example above: the school is derived from the caller, never a path parameter (`POST /school/students/promotions`). An open, pre-existing exposure that defeats its cross-school guarantee is recorded in `DEC-SCOPE-020`.
