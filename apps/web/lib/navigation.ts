...
export const SCHOOL_NAV: Record<string, NavItem[]> = {
  coordinator: ["dashboard", "students", "promotion", "activities", "team", "reports", "entitlements"].map(x => ({ label: x.replaceAll("-", " ").replace(/\b\w/g, c => c.toUpperCase()), href: `/school/coordinator/${x}` })),
...
export const PORTAL_NAV:Record<string,NavItem[]> = {
  ...
  "overseas/admin": ["dashboard","users","students","counselors","agents","commissions","universities","schools","school-staff","school-applications","applications","leads","payments","reports"].map(x=>({label:x.replaceAll("-"," ").replace(/\b\w/g,c=>c.toUpperCase()),href:`/overseas/admin/${x}`})),
