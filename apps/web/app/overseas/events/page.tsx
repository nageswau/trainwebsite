...
export default async function Events() {
  ...
  const items = events.map(event => ({
    ...
    content: <div className="card"><span className="badge">{event.event_type}</span><h3 style={{marginTop: 12}}>{event.title}</h3><p className="muted"><LocalTime value={event.starts_at} time label /> · {event.location}</p><p>{event.description}</p><a className="btn small" href={`/overseas/events/${event.id}`}>View & register</a></div>,
  }));
