...
export default async function SchoolPrincipalStudent360Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return (
    <PortalShell nav={SCHOOL_NAV.principal} roleLabel="Principal" userName={user.full_name}>
      <Student360View data={data} initialTab={tab} backHref={`/school/principal/students/${id}`} backLabel="Back to student" />
    </PortalShell>
  );
}
