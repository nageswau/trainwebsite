...
export default async function Careers() {
  ...
  const items = jobs.map(job => ({
    ...
    content: <div className="card"><span className="badge">Open</span><h3 style={{marginTop: 12}}>{job.title}</h3><p><strong>{job.company}</strong> · {job.location}</p><p className="muted">Skills: {(job.skills || []).join(", ")}</p><p className="muted">Closes: {job.closes_on ? new Date(job.closes_on).toLocaleDateString("en-GB") : "Open until filled"}</p><CareerApply jobId={job.id} jobTitle={job.title}/></div>,
  }));
